...
- **Error response uses `statusMessage` not `message`**: The live 400 response body contains `{ "status": "FAILURE_BAD_HEADER", "statusMessage": "..." }`. The spec's `ErrorResponse` schema uses `statusMessage` (corrected from the Apiary-documented `message` — issue #51).
