...
| `ANAPLAN_OAUTH_DEVICE_KEYRING_SERVICE` | `scripts/oauth/oauth_device_step2.py` | Keyring service name under which the Device grant token blob is stored (default `anaplan-oauth-device`). Kept separate from `ANAPLAN_OAUTH_KEYRING_SERVICE` so the two flows do not overwrite each other. |
