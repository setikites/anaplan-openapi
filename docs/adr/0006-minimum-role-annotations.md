...
Administrator, Restricted Integration User).
`Standard User`
...
CloudWorks has a second, higher role — Integration Admin (tenant-wide) — but it
is deliberately **not** in the vocabulary: live testing (July 2026) confirmed
`Restricted Integration User` reaches every CloudWorks endpoint, and Integration
Admin gates no additional one — it only widens *data scope* (tenant-wide vs
assigned-workspace), which is not what `x-anaplan-min-role` records. Since no
operation's minimum role is Integration Admin, adding it as an accepted value
would be dead flexibility. If a future spec (e.g. Data Orchestrator) has
operations that genuinely require it, add it to the vocabulary then.
