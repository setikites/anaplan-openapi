...
| `POST /oauth/token` (refresh) | ✓ | ✓ | Tested via `scripts/oauth/oauth_authcode_refresh.py`, which reads the refresh token from the keyring and rotates the stored token in place |
